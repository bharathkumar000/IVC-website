            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {events.map((event, index) => (
                        <ScrollReveal
                            key={event.id}
                            effect={index % 2 === 0 ? 'fadeLeft' : 'fadeRight'}
                            delay={index * 0.1}
                        >
                            <motion.div
                            ...
                            >
                                <div className="h-64 relative overflow-hidden">
                                    <div className="absolute top-3 right-3 glass-panel px-3 py-1 rounded-full text-xs font-bold tracking-wider text-black dark:text-white z-10 uppercase backdrop-blur-md border-white/20">
                                        {event.date}
                                    </div>
                                ...
                                <div className="p-8 flex flex-col flex-grow relative">
                                    <h3 className="text-2xl font-display font-bold mb-3 text-black dark:text-white group-hover:text-ivc-primary transition-colors text-glow-purple group-hover:text-glow">{event.title}</h3>
                                    <p className="text-gray-600 dark:text-gray-400 text-sm mb-6 flex-grow line-clamp-3 font-light leading-relaxed">{event.description}</p>
